...
export interface TollCost {
  tag: number;   // electronic
  cash: number;  // license plate / cash
}

interface TollGuruRequest {
  from: { address: string };
  to: { address: string };
...
function getMockTollCosts(from: string, to: string): TollCost {
  ...
  const route = routes.find(r => 
    (r.from.toLowerCase().includes(from.toLowerCase().split(',')[0]) ||
...
export async function getTollCost(opts: {
  from: string;
  to: string;
  waypoints?: string[];     // optional
  vehicleType?: string;     // default "2AxlesAuto"
  departureTime?: string;   // ISO8601, default "now"
