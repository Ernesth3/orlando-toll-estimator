...
import { getTollCost, TollCost } from '../tollguru';
...
router.post('/estimate', async (req, res) => {
  try {
    ...
    function calendarDaysInclusive(start: string | Date, end: string | Date) {
    ...
    function toETDateKey(iso: string) {
      // 用 Intl 即可，无需引第三方库
    ...
    function computeTotals() {
      const tollDaysET = new Set<string>();
      let cashTollsTotal = 0;
